...
class SelectConstructor {
	...
	getSelectTitleValue(selectItem, originalSelect) {
		...
		if (originalSelect.multiple && originalSelect.hasAttribute('data-tags')) {
			// Генерируем теги для внешнего контейнера
			const tagsHTML = this.getSelectedOptionsData(originalSelect).elements.map(option =>
			...
			// Выводим теги во внешний блок
			if (originalSelect.dataset.tags && document.querySelector(originalSelect.dataset.tags)) {
				document.querySelector(originalSelect.dataset.tags).innerHTML = tagsHTML;
			}

			// Для select__content оставляем просто плейсхолдер или количество выбранных элементов
