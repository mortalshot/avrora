(() => {
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".js-icon-menu")) document.addEventListener("click", (function(e) {
        ...
        class SelectConstructor {
            ...
            getSelectTitleValue(selectItem, originalSelect) {
                ...
                if (originalSelect.multiple && originalSelect.hasAttribute("data-tags")) {
                    const tagsHTML = this.getSelectedOptionsData(originalSelect).elements.map((option => `<span role="button" data-select-id="${selectItem.dataset.id}" data-value="${option.value}" class="_select-tag">\n                ${this.getSelectElementContent(option)}\n            </span>`)).join("");
                    if (originalSelect.dataset.tags && document.querySelector(originalSelect.dataset.tags)) document.querySelector(originalSelect.dataset.tags).innerHTML = tagsHTML;
                    selectTitleValue = originalSelect.dataset.placeholder || `Выбрано: ${this.getSelectedOptionsData(originalSelect).values.length}`;
                }
        ...
        function initSliders() {
            if (document.querySelector(".homebanner__slider")) new Swiper(".homebanner__slider", {
                speed: 800,
                spaceBetween: 40,
                watchOverflow: true,
                autoHeight: true,
            ...
            } else new Swiper(".product-gallery__main", {
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                spaceBetween: 20,
            ...
            if (document.querySelector(".team-widget__slider")) new Swiper(".team-widget__slider", {
                speed: 800,
                slidesPerView: 2,
                spaceBetween: 10,
                watchOverflow: true,
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            function updateNav(currentIndex) {
                ...
                navItems.forEach(((item, index) => {
                    if (index === currentIndex) item.classList.add("_current"); else if (index < currentIndex) item.classList.add("_done");
                }));
            }
            function changeStep(newIndex) {
                const currentStep = document.querySelector(".brief__step._active");
                const currentIndex = Array.from(steps).indexOf(currentStep);
                if (newIndex > currentIndex && !validateStep(currentStep)) return false;
            ...
            navItems.forEach(((item, index) => {
                item.addEventListener("click", (() => {
                    ...
                    if (index !== currentIndex && (index < currentIndex || validateStep(steps[currentIndex]))) changeStep(index);
                }));
            }));
        ...
        function initFileItem(fileItem) {
            ...
            fileItem.addEventListener("drop", (function(e) {
                ...
                if (e.dataTransfer.files.length) {
                    ...
                    for (let file of newFiles) if (file.size > 10 * 1024 * 1024) {
